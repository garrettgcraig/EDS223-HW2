...
#| fig-width: 10
#| fig-height: 8
#| fig-cap: "**Historical redlining in Los Angeles County.** HOLC grades from the 1930s are shown: Grade A (green) represents 'best' neighborhoods, Grade B (blue) 'still desirable', Grade C (yellow) 'definitely declining', and Grade D (red) 'hazardous'."
...
```{r holc-summary}
#| tbl-cap: "**Census block groups by HOLC grade.** Distribution across HOLC grades in Los Angeles County."
...
#| fig-cap: "**Environmental and socioeconomic conditions by HOLC grade.** Mean values show Grade D (redlined) neighborhoods have higher percentages of low-income residents, higher PM2.5 exposure, and lower life expectancy compared to Grade A neighborhoods."
...
env_long <- env_summary %>%
  ...
  mutate(
    variable = case_when(
      ...
      variable == "mean_life_exp" ~ "Low Life Exp. Percentile"
    )
  )
...
ggplot(env_long, aes(x = grade, y = value, fill = grade)) +
  ...
  labs(
    title = "Env. and Socioeconomic Conditions by HOLC Grade",
...
```{r bird-observations}
#| fig-cap: "**Bird observations by HOLC grade.** Distribution of observations across HOLC grades in Los Angeles County (2021-2023). Grade C neighborhoods have the highest percentage of observations, while Grade D (redlined) neighborhoods have the lowest."
...
Several factors could explain why our results differ from the paper's findings. First, there is an important methodological difference: Ellis-Soto et al. used log-transformed observation counts in their analysis, while our analysis uses raw counts and percentages. Log transformation is commonly applied to biodiversity count data because observation numbers are often highly skewed, with some areas having very high counts and others having very few. By log-transforming the data, their analysis would have given different weight to the differences between HOLC grades and could reveal different patterns in the relationship between historical redlining and current observation densities. This transformation helps normalize the distribution and makes relationships more linear, which could explain why our raw count analysis shows different patterns.

Second, Ellis-Soto et al. examined not just the quantity of observations but also species richness, observer effort, and temporal trends across multiple cities. Our analysis focuses solely on the count of observations in Los Angeles, which may reflect observer behavior and accessibility rather than actual biodiversity patterns. Grade C and B neighborhoods may have more accessible green spaces or parks that attract both birds and birders, while grade D neighborhoods may have fewer observers despite potentially having birds present.
